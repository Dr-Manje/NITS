...
if(isset($_POST['addnewDispatchData'])){
    ...
    if(strtolower(end($chk_ext)) == "csv")
    {
        ...
        {               
            ...
            $cg7Received = $data[5]; //confirmed
            $chalimReceived = $data[6]; //confirmed by
            $notes = $data[7]; //notes
            ...
            $add = $seasons->addNewDispatchTrip($date,$departure,$destination,$cg7,$chalim,$cg7Received,$chalimReceived,$notes,$status,$seasonid,$_SESSION['nasfam_userIPC']);
...
if(isset($_POST['addnewGradingData'])){
    ...
    if(strtolower(end($chk_ext)) == "csv")
    {
        ...
        {               
            ...
            $addGradingData = $seasons->addGradingData($date,$warehouse,$seasonid,$variety,$grade,$qty,$_SESSION['nasfam_userIPC']);
        }
        fclose($handle);
        $_SESSION['notification']['title'] = 'SUCCESS!';
...
if(isset($_POST['updateseasonheader'])){
    ...
    $CheckYearExist = $dashboard->CheckYearExist1($startdate1,$enddate1,$id);
        if($CheckYearExist >= 1){ //exists, notify user
            ...
            echo 'doesnt exist';
            $updateseason = $dashboard->updateseason($start,$end,$season,$id);
            $_SESSION['notification']['title'] = 'PASSED!';
...
if(isset($_POST['addnewBuyers'])){
    ...
    if(strtolower(end($chk_ext)) == "csv")
    {
        ...
        {               
            ...
            $addNewBuyer = $seasons->addNewBuyer($mkcode,$names,$lnames,$gender,$contacts,$seasonid,$newNumber,$_SESSION['nasfam_userIPC']);               
        }
        fclose($handle);
        $_SESSION['notification']['title'] = 'SUCCESS!';
...
if(isset($_POST['InsertAdvance'])){
    ...
    $fname = $_FILES['file']['name'];
        ...
        if(strtolower(end($chk_ext)) == "csv")
        {
            ...
            while(($data = fgetcsv($handle, 1000, ",")) !== false)
            {               
                $advancetype = $data[0]; //advance type
                $receipient = $data[1]; //receipient
                $amount = $data[2]; //advance amount
                
                $date = $data[3]; //date
                $remarks = $data[4]; //remarks
                $a = explode("/", $date);
                //2017-12-31 00:00:00
                $advanceDate = $a[2].'-'.$a[1].'-'.$a[0].' 00:00:00';
                
                switch ($advancetype) {
                    case "IPC":
                        //insert into IPC
                        $ipc = $seasons->GetIPC($receipient);
                        $advanceTyping = 1;
                        //$message .= $advancetype.' '.$ipc.' '.$amount.' '.$advanceDate.'<br>';
                        $insertAdvance = $seasons->insertAdvanceIPC($amount,$advanceTyping,$ipc,$seasonid,$advanceDate,$remarks);
                        break;
                    case "MKC":
                        //insert into MKC
                        $ipc = $seasons->GetMKC($receipient);
                        $advanceTyping = 2;
                        //$message .= $advancetype.' '.$ipc.' '.$amount.' '.$advanceDate.'<br>';
                        $insertAdvance = $seasons->insertAdvanceMKC($amount,$advanceTyping,$ipc,$seasonid,$advanceDate,$remarks,$_SESSION['nasfam_userIPC']);
                        break;
                    default:
                        $message .= 'Neither of the above';
                }
...
//            exit();
            ...
            $_SESSION['notification']['title'] = 'FAILED!';
            $_SESSION['notification']['message'] = 'Something went wrong! Please try again, if this problem persists, please contact your admin';
            header("Location: seasondetails.php?sid=$seasonid");
            exit();
        }
...
if(isset($_POST['addNewMarketCenter'])){
    ...
    $addMarketCenter = $seasons->addMarketCenter($marketcentername,$newNumber,$seasonid,$_SESSION['nasfam_userIPC']);
        ...
        if(strtolower(end($chk_ext)) == "csv")
        {
            ...
            {               
                ...
                if($member == 'member'){//none member purchase
                    ...
                    $farmerstatus = '1';
                    $addMemberPurchase = $seasons->addMemberPurchase($receipt,$date,$buyerID,$farmerstatus,$type,$qty,$price,$mwk,$seasonid,$farmer,$_SESSION['nasfam_userIPC']);
                }else{ //member purchase  
                    $farmer = strtoupper($data[2]); //farmer
                    $farmerstatus = '0';
                    $addNoneMemberPurchase = $seasons->addNoneMemberPurchase($receipt,$date,$buyerID,$farmer,$gender,$farmerstatus,$type,$qty,$price,$mwk,$seasonid,$_SESSION['nasfam_userIPC']);
                }
...
if(isset($_POST['addSorting'])){
    ...
    if(strtolower(end($chk_ext1)) == "csv")
    {
        ...
        while(($data1 = fgetcsv($handle1, 1000, ",")) !== false)
        {
            ...
            $addSorting = $seasons->addSorting($casualworker,$date,$qty,$variety,$gradeouts,$shells,$seasonid,$_SESSION['nasfam_userIPC']);
...
if(isset($_POST['addCasualWorkers'])){
    ...
    if(strtolower(end($chk_ext1)) == "csv")
    {
        ...
        while(($data1 = fgetcsv($handle1, 1000, ",")) !== false)
        {
            ...
            $addCasualworker = $seasons->addCasualworker($whs,$names,$lname,$gender,$newNumber2,$seasonid,$_SESSION['nasfam_userIPC']);               
        }
        fclose($handle1);
        $_SESSION['notification']['title'] = 'SUCCESS!';
...
if(isset($_GET['sid'])){
    ...
    if($_SESSION['nasfam_usertype'] == '1'){
        ...
        $getSeasonHeader = $seasons->SeasonDetails($id,$_SESSION['nasfam_userIPC']);
        
        $getSeasonNumbers = $seasons->getSeasonVariance($id,$_SESSION['nasfam_userIPC']);//get variance and total spent
        $variance = $getSeasonNumbers[0][1]; //variance
        $totalspent = $getSeasonNumbers[0][0]; //total spent
        //advance to date
        
        $lstAllMarketCenterListData = $seasons->lstAllMarketCenterList1($id,$_SESSION['nasfam_userIPC']);
        //$lstIPCAdvanceListData = $seasons->lstAllMarketCenterList1($id);
        
        //casual worker data
        $lstCasualWorkersData = $seasons->lstCasualWorkersData($id,$_SESSION['nasfam_userIPC']);//select list of sorting data
        
        $lstBuyersData = $seasons->lstAllBuyersData($id,$_SESSION['nasfam_userIPC']); //buyers data
        
        $lstPurchasesList = $seasons->lstPurchasesList($id,$_SESSION['nasfam_userIPC']);//purchase list
        
        $warehouseData = $seasons->lstAllWarehouse($id,$_SESSION['nasfam_userIPC']); // warehouse data
        
        $lstGradingData = $seasons->getGradingWarehouse();//select list of sorting data
        
        $seasonMarketCenters = $seasons->seasonMarketCenters($id,$_SESSION['nasfam_userIPC']); //select All market centers from this season
        
        $lstDispatchList = $seasons->getDispatchList($id,$_SESSION['nasfam_userIPC']); //select list of dispatch data
        
        //advance data
        $ListIPCadvance = $seasons->ListIPCadvance($id,$_SESSION['nasfam_userIPC']); //IPC Advance
        $ListMKCadvance = $seasons->ListMKCadvance($id,$_SESSION['nasfam_userIPC']); //MKC advance
    }
    //dash items
...
//        }
        ...
        $getTotalAdvanceMKC = $seasons->getTotalAdvanceMKC($cwid);
        
        //$Balance = $mpa - $ReceiptTotal;
        $firstReport = $seasons->seasonfirstReport($cwid);
        $totalKGs = $firstReport[0][0];
        //$receipts = $firstReport[0][1];
        
        $advance = $getTotalAdvanceMKC - $ReceiptTotal;//get advance to date
        
        array_push($memberinfo, $marketcenter);
        array_push($memberinfo, $code);
        array_push($memberinfo, $Gacs);
        array_push($memberinfo, $advance); //advance to date
        array_push($memberinfo, $stat);
        array_push($memberinfo, $ReceiptTotal);
        array_push($memberinfo, $totalKGs);
...
//        }
    ...
    foreach($lstGradingData as $value){
        ...
        $CG7g1 = $seasons->getGradingVarietyGradingData('CG7','1',$whsID,$id,$_SESSION['nasfam_userIPC']); array_push($memberinfo, $CG7g1); //CG 7 g 1
        $CG7g2 = $seasons->getGradingVarietyGradingData('CG7','2',$whsID,$id,$_SESSION['nasfam_userIPC']); array_push($memberinfo, $CG7g2); //CG 7 g 2
        $CG7g3 = $seasons->getGradingVarietyGradingData('CG7','3',$whsID,$id,$_SESSION['nasfam_userIPC']); array_push($memberinfo, $CG7g3); //CG 7 g 3
        $CG7g4 = $seasons->getGradingVarietyGradingData('CG7','4',$whsID,$id,$_SESSION['nasfam_userIPC']); array_push($memberinfo, $CG7g4); //CG 7 g 4
        $CG7g5 = $seasons->getGradingVarietyGradingData('CG7','5',$whsID,$id,$_SESSION['nasfam_userIPC']); array_push($memberinfo, $CG7g5); //CG 7 g 5
        $CG7total = $CG7g1+$CG7g2+$CG7g3+$CG7g4+$CG7g5; array_push($memberinfo, $CG7total); //CG 7 total
        $CHALIMg1 = $seasons->getGradingVarietyGradingData('CHALIM','1',$whsID,$id,$_SESSION['nasfam_userIPC']); array_push($memberinfo, $CHALIMg1); //CHALIM g 1
        $CHALIMg2 = $seasons->getGradingVarietyGradingData('CHALIM','2',$whsID,$id,$_SESSION['nasfam_userIPC']); array_push($memberinfo, $CHALIMg2); //CHALIM g 2
        $CHALIMg3 = $seasons->getGradingVarietyGradingData('CHALIM','3',$whsID,$id,$_SESSION['nasfam_userIPC']); array_push($memberinfo, $CHALIMg3); //CHALIM g 3
        $CHALIMg4 = $seasons->getGradingVarietyGradingData('CHALIM','4',$whsID,$id,$_SESSION['nasfam_userIPC']); array_push($memberinfo, $CHALIMg4); //CHALIM g 4
        $CHALIMg5 = $seasons->getGradingVarietyGradingData('CHALIM','5',$whsID,$id,$_SESSION['nasfam_userIPC']); array_push($memberinfo, $CHALIMg5); //CHALIM g 5
    ...
    //Dispatch
    $lstDispatch = array();
    foreach($lstDispatchList as $value){
        $memberinfo = array();
        $dID = $value['did']; //warehouse id
        $season = $value['season']; //warehouse name

        array_push($memberinfo, $value['departuredate']); //warehouse
        array_push($memberinfo, $value['departure']); //CG 7 g 1
        array_push($memberinfo, $value['destination']); //CG 7 g 1
        
        array_push($memberinfo, $value['cg7_Sent']); //CG 7 g 1
        array_push($memberinfo, $value['chalim_sent']); //CG 7 g 1
        
        $totalSent = $value['cg7_Sent'] + $value['chalim_sent'];
        array_push($memberinfo, $totalSent); //CG 7 g 1
        
        array_push($memberinfo, $value['cg7_received']); //CG 7 g 1
        array_push($memberinfo, $value['chalim_received']); //CG 7 g 1
        
        $totalReceived = $value['cg7_received'] + $value['chalim_received'];
        array_push($memberinfo, $totalReceived); //CG 7 g 1
        
        array_push($memberinfo, $value['status']); //CG 7 g 1
        array_push($memberinfo, $value['notes']); //CG 7 g 1

        //$actiontime = '<a href="gradingdetails.php?gradid='.$whsID.'&seasonid='.$id.'" rel="tooltip" title="View Grading Details" class="btn btn-success btn-xs" ><i class="fa fa-view"></i> VIEW</a>'; 
        
        $actiontime = '<button onclick="dispatchUpdateModal('.$dID.')" rel="tooltip" title="Edit/Update Dispatch" class="btn btn-info btn-xs" ><i class="fa fa-edit"></i> EDIT</button> '
                . '<button onclick="deleteDispatch('.$dID.')" rel="tooltip" title="Delete Dispatch" class="btn btn-danger btn-xs" ><i class="fa fa-trash"></i> DELETE</button>';
        
        //$actiontime = 'Action item';
        
        array_push($memberinfo, $actiontime); //action
        
        array_push($lstDispatch, $memberinfo);
    }
...
if(isset($_POST['SearchDistrictReg'])){
    $seasonid = $_POST['regyearDS']; //market center name season id
    //get reg year then redirect
    //$_SESSION['notification']['title'] = 'SUCCESS!';
    //$_SESSION['notification']['message'] = 'Sorting Data Successfuly added!';
    header("Location: seasondetails.php?sid=$seasonid");
    exit();
